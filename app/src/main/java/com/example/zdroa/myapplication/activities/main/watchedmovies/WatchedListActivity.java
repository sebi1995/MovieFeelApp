package com.example.zdroa.myapplication.activities.main.watchedmovies;
...
import com.example.zdroa.myapplication.activities.main.movies.MoviesListViewAdaptor;
...
public class WatchedListActivity extends AppCompatActivity {
    ...
    UserSessionHandler session_handler;
...
//        watchedList = new HashMap<>(new ConvertDbIdsToHashMapII().getHasMap(session_handler.getWatchedList()));
        ...
        HttpRequestRunner.executeAsync(new MovieGetter(0), result -> {
            MoviesListViewAdaptor adapter = new MoviesListViewAdaptor(fa, result);
            ...
            if (next_b) {
                ivNext.setImageResource(R.mipmap.ic_next_results_active);
            } else ivNext.setImageResource(R.mipmap.ic_next_results_inactive);
            ivPrevious.setEnabled(previous_b);
            if (previous_b) {
                ivPrevious.setImageResource(R.mipmap.ic_previous_results_active);
            } else ivPrevious.setImageResource(R.mipmap.ic_previous_results_inactive);
        ...
        listView.setOnItemClickListener((parent, view, position, id) -> {
            startActivity(new Intent(WatchedListActivity.this, WatchedDetailsActivity.class)
                    .putExtra("id", IDS_AFTER_QUERY.get(position))
                    .putExtra("title", TITLES.get(position))
                    .putExtra("poster", POSTERS.get(position))
                    .putExtra("adult", ADULT.get(position))
                    .putExtra("overview", OVERVIEW.get(position))
                    .putExtra("runtime", RUNTIME.get(position))
                    .putExtra("status", STATUS.get(position))
                    .putExtra("release_date", RELEASE_DATE.get(position))
                    .putExtra("vote_average", VOTE_AVERAGE.get(position))
                    .putExtra("original_language", ORIGINAL_LANGUAGE.get(position))
                    .putStringArrayListExtra("genres", new ArrayList<>(GENRES.get(position).values()))
                    .putStringArrayListExtra("production_countries", new ArrayList<>(PRODUCTION_COUNTRIES.get(position).values()))
                    .putStringArrayListExtra("spoken_languages", new ArrayList<>(SPOKEN_LANGUAGES.get(position).values()))
                    .putStringArrayListExtra("youtube_links", new ArrayList<>(YOUTUBE_LINKS.get(position).values())));
        });
    ...
    private void disableInterface() {
        ivNext.setImageResource(R.mipmap.ic_next_results_active);
        ivNext.setEnabled(false);
        ivPrevious.setImageResource(R.mipmap.ic_previous_results_inactive);
